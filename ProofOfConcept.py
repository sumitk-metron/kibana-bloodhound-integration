...
def create_kibana_case(finding: dict) -> dict:
    """POST https://localhost:5601/api/cases  →  creates a Kibana case from a finding."""
    title = finding.get("title", "Untitled Case")
    tags = [finding.get("type", "bloodhound")]
    description = (
        f"## Short Remediation\n\n{finding.get('short_remediation', '')}\n\n"
        f"## Long Remediation\n\n{finding.get('long_remediation', '')}"
    )
...
def main():
    ...
    available_types = get_available_types(
        session_token, target_domain_id, asset_group_tag_id=1
    )

    # Step 5 & 6 – Fetch details for each available type and create cases
    print("\n" + "#" * 60)
    print("  KIBANA — Create Cases for all Issue Types")
    print("#" * 60)

    for finding_type in available_types.get("data", []):
        if not isinstance(finding_type, str):
            continue

        print(f"\n[main] Fetching details for issue type: {finding_type}")
        issue_details = get_issue_details(session_token, finding_type=finding_type)

        findings = issue_details.get("data", [])
        if isinstance(findings, list):
            for finding in findings:
                create_kibana_case(finding)
        elif findings:
            create_kibana_case(findings)
